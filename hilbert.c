 ...
 */
bitmask_t
hilbert_c2i_result(unsigned nDims, unsigned nBits, bitmask_t const coord[], const size_t mode)
{
	// printf("in the result sequence.............\n");
...
{
		// printf("in the REGULAR sequence.............\n");
  ...
  for (x.d = c[d=0]; d < nDims; x.d = c[++d])
    {
      ...
      if (diff <= (unsigned) 52u)
	bit ^= 1 & ((diff < (unsigned) 32u)? x.ieee.mantissa1 >> diff:
		    (diff < (unsigned) 52u)? x.ieee.mantissa0 >> (unsigned) (diff - 32) :
		    /* else */    normalized);
      else
	bit ^= (y == IEEErepBits-1);
		  ...
		  char* cP, unsigned y, int fold)
{
  ...
  else if (diff == 52u) /* "flip" the implicit 1 bit */
    {
      if (normalized)
	--x->ieee.exponent;
      else
  ...
  for (b = nBits; b--;)
    {
      ...
      // printf("bits=%u, reflection=0\n", bits);
      reflection = 0;
      for (d = 0; d < nDims; ++d) {
      	// printf("rdbit here gets the following result %zu\n", rdbit(coord[d], b));
        reflection |= rdbit(coord[d], b) << d;
        // printf("reflection updated to %u (given this coord[d]==%u) and its rdbit result\n", reflection, coord[d]);
      }
      bits ^= reflection;
      // printf("bits logAND with reflection to %u\n", bits);
      bits = rotateRight(bits, rotation, nDims);
      // printf("Rotate bits to the right\n");
      index ^= bits;
      // printf("index logAND with bits to %u\n", index);
      for (d = 1; d < nDims; d *= 2) {
      	// printf("d before %u\n", d);
     	index ^= index >> d;
     	// printf("d after %u\n", d);
     	// printf("for d=(either 1,2,4,8...) index logAND with index>>d so %u\n", index);
      }
      if (index++ != ndOnes)
        {
          // printf("do something special until index reaches 0s\n");
          rb = b;
          rd = index & -index;
      ...
      index &= 1;
      index <<= nDims-1;
      // printf("again, modify index to %u\n", index);
      reflection ^= one << rotation;
      // printf("update reflection to %u (with one)\n", reflection);
      adjust_rotation(rotation,nDims,bits);
      // printf("adjust rotation which updated bits to %u and rotation to %u\n", bits, rotation);
    }

  // printf("We execute this line which updates coord\n");
  for (d = 0; !rdbit(rd, d); ++d) {
  	// printf("d=%u, rdbit is a condition\n", d);
  }
  // bitmask_t old_coord = coord[d];
  coord[d] ^= (2 << rb) - 1;
  // printf("\n");
  // if (d!=0) {
	 //  if (old_coord<coord[d]) {
	 //  	printf("increment result_size\n");
	 //  } else {
	 //  	printf("decrement result_size\n");
	 //  }
  // } else {
  // 	printf("We moved on the mode; reset\n");
  // }
}
